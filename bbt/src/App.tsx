...
import { Home, Trophy, User as UserIcon, Settings as SettingsIcon, X } from 'lucide-react';
...
import { coralBayCity, getDistrict } from './data/cityMapData';
...
const POOL_CEILING = 5500000; // $5.5M — tuned via simulation to land completion around 45-50 days for a consistent 4x/day player.
...
function AppInner({ currentUid }: { currentUid: string }) {
  ...
  const currentDistrictMeta = getDistrict(coralBayCity, currentDistrictId);
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'city' | 'leaderboard' | 'profile'>('city');
  ...
  useEffect(() => {
    const tickInterval = setInterval(() => {
      setStats((prev) => {
        ...
        // $5.5M ceiling (POOL_CEILING) — miss a claim for a while and
  ...
  useEffect(() => {
    coralBayCity.districts.forEach((district) => {
  ...
  useEffect(() => {
    coralBayCity.districts.forEach((district) => {
      ...
      setMilestone({
        ...
        bonusText: `Earned +$${scaledReward.toLocaleString('en-US')} completion bonus`,
        color: 'green',
      });
      setShowConfetti(true);
  ...
  const displayedDistrictMeta = getDistrict(coralBayCity, displayedDistrictId);
  ...
  const districtProgressMap = useMemo(() => {
    ...
    coralBayCity.districts.forEach((d) => {
  ...
  return (
    <div className="app-shell-height md:min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-start md:justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
      ...
      <div className="absolute top-12 left-12 w-72 pointer-events-none hidden xl:block space-y-4 font-sans">
        <div className="flex items-center gap-2" style={{ color: 'var(--color-premium-gold-600)' }}>
          <Landmark className="animate-spin" style={{ animationDuration: '10s' }} />
          <h1 className="font-display font-extrabold text-xl tracking-tight uppercase" style={{ color: 'var(--color-ink-900)' }}>
            Coral Bay Resort
          </h1>
        </div>
        <p className="text-xs leading-relaxed font-semibold" style={{ color: 'var(--color-ink-700)' }}>
      ...
      {/* HIGH-FIDELITY MOBILE DEVICE MOCKUP FRAME CONTAINER (FLAT INK FRAME) */}
      <div className="app-shell-height relative w-full md:h-[880px] md:max-w-[420px] bg-[var(--color-ink-900)] md:rounded-[42px] md:border-[10px] md:border-[var(--color-premium-gold-400)] md:shadow-[8px_10px_0_rgba(27,27,47,0.35)] flex flex-col overflow-hidden">
        
        {/* Mobile Camera notch and status strip (Visible on desktop mockup shell only) */}
        <div className="hidden md:flex w-full h-8 bg-[var(--color-ink-900)] justify-between items-center px-6 text-[var(--color-premium-gold-100)]/80 text-[10px] font-mono font-bold select-none relative border-b border-[var(--color-ink-700)]">
          {/* Simulated Time */}
          <span>9:41 AM</span>
          
          {/* Central Notch speaker capsule */}
          <div className="absolute top-1.5 left-1/2 -translate-x-1/2 w-24 h-4 bg-black rounded-full flex items-center justify-center border border-white/10">
          ...
          <div className="flex items-center gap-1.5">
            <span>5G</span>
            <span>📶</span>
            <span className="text-[var(--color-premium-green-500)]">100% 🔋</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar relative pb-6" style={{ backgroundColor: 'var(--color-premium-bg)' }}>
            ...
            >
              {activeTab === 'home' && (
                <div className="min-h-full flex flex-col gap-1.5 px-3 pt-1 pb-2">
                  <button
                    onClick={() => { playClick(); setActiveTab('city'); }}
                    className="self-start flex items-center gap-1.5 mb-1 px-3 py-1.5 rounded-[14px] cursor-pointer"
                    style={{ background: 'var(--color-premium-surface)', border: 'none', boxShadow: '0 6px 16px rgba(34,49,66,0.14)', color: 'var(--color-premium-text)' }}
                  >
                    <Home size={14} strokeWidth={3} />
                    <span className="text-[11px] font-extrabold uppercase tracking-wide">Map</span>
                  </button>
                  {isPreviewMode ? (
              ...
              {activeTab === 'city' && (
                <div className="absolute inset-0">
                  ...
                  {/* Floating map controls — replaces the old persistent bottom
                      tab bar entirely. The map is the permanent home screen;
                      these three chunky circular buttons are the only way to
                      reach Portfolio, Ranks, and Settings. */}
                  <div className="absolute z-40 flex flex-col gap-2.5" style={{ top: '0.75rem', right: '0.75rem' }}>
                    {[
                      { label: 'Ranks', icon: Trophy, action: () => { playClick(); setActiveTab('leaderboard'); }, bg: 'var(--color-premium-info-400)' },
                      { label: 'Portfolio', icon: UserIcon, action: () => { playClick(); setActiveTab('profile'); }, bg: 'var(--color-premium-gold-400)' },
                      { label: 'Settings', icon: SettingsIcon, action: () => { playClick(); setShowSettings(true); }, bg: 'var(--color-premium-surface)' },
                    ].map((btn) => (
                      <button
                        key={btn.label}
                        onClick={btn.action}
                        ...
                        style={{ background: btn.bg, border: 'none', boxShadow: '0 8px 20px rgba(34,49,66,0.18)', color: btn.bg === 'var(--color-premium-surface)' ? 'var(--color-premium-text)' : 'var(--color-premium-text-inverse)' }}
                      >
                        <btn.icon size={20} strokeWidth={2.75} />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'leaderboard' && (
                <div className="min-h-full relative px-1 pt-1 pb-4">
                  <button
                    onClick={() => { playClick(); setActiveTab('city'); }}
                    className="flex items-center gap-1.5 mb-2 ml-2 px-3 py-1.5 rounded-[14px] cursor-pointer"
                    style={{ background: 'var(--color-premium-surface)', border: 'none', boxShadow: '0 6px 16px rgba(34,49,66,0.14)', color: 'var(--color-premium-text)' }}
                  >
                    <X size={14} strokeWidth={3} />
                    <span className="text-[11px] font-extrabold uppercase tracking-wide">Back to Map</span>
                  </button>
                  <LeaderboardTab
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 select-none">
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-slate-950">
                  ...
                  <h2 className="font-bold text-lg text-white uppercase tracking-wide">Become the Coral Bay Kahuna!</h2>
                ...
                <div className="p-4 bg-gradient-to-t from-black/80 to-transparent text-center relative z-10">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Thank you for supporting Coral Bay Resort Tycoon!</span>
                </div>
              </motion.div>
            </div>
          )}
