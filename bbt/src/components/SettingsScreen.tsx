        ...
        >
          ...
          <div className="p-4 space-y-4 pb-10">
            ...
            <Card>
              ...
              <Divider />
              <NavRow icon={Shield} label="Privacy Policy" onNavigate={() => setShowPrivacyPolicy(true)} />
              <Divider />
          ...
          <PrivacyPolicyScreen isOpen={showPrivacyPolicy} onClose={() => setShowPrivacyPolicy(false)} />
        </motion.div>
      )}
    </AnimatePresence>
  );
...
const NavRow: React.FC<{
  ...
  label: string;
  value?: string;
  /** Called after the click sound, for rows that actually navigate
   *  somewhere (Privacy Policy) rather than being purely decorative. */
  onNavigate?: () => void;
}> = ({ icon, label, value, onNavigate }) => (
  <button onClick={() => { playClick(); onNavigate?.(); }} className="w-full flex items-center gap-3 px-3.5 py-3 cursor-pointer">
    <RowIcon icon={icon} />
    <span className="flex-1 text-left font-medium text-[12px]" style={{ color: 'var(--color-premium-text)' }}>
      {label}
    </span>
