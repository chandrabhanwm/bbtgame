        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {topThree.map((entry, i) => (
              ...
              >
                <SpotlightCard entry={entry} rank={i + 1} isMe={entry.uid === myUid} valueType={view === 'overall' ? 'cash' : 'points'} />
              </motion.div>
            ))}
          </div>
          ...
          {remaining.length > 0 && (
            ...
            >
              {remaining.map((entry, i) => (
                <RankRow key={entry.uid} entry={entry} rank={i + 4} isLast={i === remaining.length - 1} isMe={entry.uid === myUid} valueType={view === 'overall' ? 'cash' : 'points'} />
              ))}
            </div>
          )}
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <div className="text-[10px] mt-0.5" style={{ color: 'var(--color-premium-text-secondary)' }}>
            Resets Monday — points back to zero
          </div>
        </div>
      </div>
