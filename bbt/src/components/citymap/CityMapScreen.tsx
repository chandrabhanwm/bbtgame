...
import { coralBayCity, getDistrict, isRoadActive, District } from '../../data/cityMapData';
...
export const CityMapScreen: React.FC<CityMapScreenProps> = ({ onOpenDistrict, onPreviewDistrict, isDistrictUnlocked, districtProgress, businessesByDistrict, celebratingDistrictId, currentDistrictId }) => {
  ...
  const checkUnlocked = (districtId: string): boolean => {
    ...
    return getDistrict(coralBayCity, districtId)?.unlocked ?? false; // backward-compatible fallback
  };
  ...
  const xs = coralBayCity.districts.map((d) => d.x);
  const ys = coralBayCity.districts.map((d) => d.y);
  const pad = 65;
      ...
      >
        ...
        >
          <svg viewBox={viewBox} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
            ...
            {coralBayCity.roads.map((road) => {
              const from = getDistrict(coralBayCity, road.from);
              const to = getDistrict(coralBayCity, road.to);
              ...
              return (
                <RoadPath
                  ...
                  from={from}
                  to={to}
                  active={isRoadActive(coralBayCity, road, checkUnlocked)}
                  traveled={traveled}
            ...
            {coralBayCity.districts.map((district) => (
              <DistrictNode
                key={district.id}
                district={district}
