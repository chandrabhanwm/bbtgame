...
const StatCell: React.FC<{ icon: React.ReactNode; label: string; value: string; valueColor?: string; last?: boolean }> = ({
  ...
  // A growing income number (₹250/min → ₹50,000/min → ₹12,50,000/min)
  // needs real room that a fixed font size can't guarantee inside a
  // 1/4-width column. Rather than truncate the actual number (hiding
  // real information) or let it overflow into the next column, the
  // font size itself shrinks as the value gets longer — the full
  // number always stays visible, just smaller when it needs to be.
  const fontSize = value.length > 11 ? '8.5px' : value.length > 8 ? '9.5px' : '11px';
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ color: valueColor ?? 'var(--color-premium-text)', fontSize }}
      >
        {icon}
        {value}
      </span>
