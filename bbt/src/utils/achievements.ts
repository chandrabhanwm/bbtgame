...
import { coralBayCity } from '../data/cityMapData';
...
const CATEGORY_BUSINESS_IDS: Record<string, string[]> = {
  'Food & Beverage': ['coconut_stand'],
  'Restaurant': ['seafood_shack'],
  'Grocery': ['beach_mart'],
  'Dairy': ['gelato_cart'],
  'Bakery': ['bakery'],
  'Automotive': ['surf_rentals'],
  'Healthcare': ['first_aid_hut'],
  'Events': ['beach_bungalow'],
};
...
export function computeAchievements(stats: PlayerStats, businessesByDistrict: Record<string, Business[]>): Achievement[] {
  ...
  // 7 category Kahuna achievements
  Object.entries(CATEGORY_BUSINESS_IDS).forEach(([category, businessIds]) => {
    ...
    achievements.push({
      id: `kahuna_${category.toLowerCase().replace(/[^a-z]/g, '_')}`,
      title: `${category} Kahuna`,
  ...
  // 3 cumulative net-worth tiers (replaces the old single net-worth badge)
